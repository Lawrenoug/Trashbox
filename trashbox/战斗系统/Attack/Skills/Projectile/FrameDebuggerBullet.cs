...
namespace Attack
{
	public partial class FrameDebuggerBullet : Bullet
	{
		...
		public override void OnBodyEntered(Node body)
		{
			if (body.IsInGroup("enemy"))
			{
				...
				ProgramDebugging buff = new ProgramDebugging();
				// 检查敌方单位身上的ProgramDebugging buff数量
				int programDebuggingCount = 0;
				foreach (Node child in enemy.GetChildren())
				{
					if (child is ProgramDebugging)
					{
						programDebuggingCount++;
					}
				}
				
				// 只有当buff数量小于最大层数时才添加新的buff
				if (programDebuggingCount < buff.layer) // ProgramDebugging的最大层数是3
				{
					enemy.AddChild(buff);
				}
